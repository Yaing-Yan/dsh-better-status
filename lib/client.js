...
// ===== pricing fallback (USD per 1M tokens) — 优先使用 Host 按当前模型返回的单价 =====
...
function costUsd(usage, pricing) {
  var p = pricing || FALLBACK_PRICING;
  ...
  return (uncached + cacheWrite) / 1e6 * p.inputUsdPerM
    + cacheRead / 1e6 * p.cacheHitUsdPerM
    + output / 1e6 * p.outputUsdPerM;
}
...
var CONTRIBUTION = { package: 'dsh-better-status', descriptors: [GET_BALANCE, GET_PRICING] };
...
exports.apply = function apply(ctx) {
  ...
  function StatusPanel(props) {
    ...
    var pricingState = React.useState(null);
    var pricing = pricingState[0];
    var setPricing = pricingState[1];
    ...
    React.useEffect(function () {
      ...
      var alive = true;
      if (typeof face.getBalance === 'function') {
      ...
      if (typeof face.getPricing === 'function') {
        face.getPricing().then(function (r) {
          if (alive && r && r.ok === true) setPricing(r.value);
        }).catch(function () {});
      }
    ...
    var spend = usage != null ? costUsd(usage, pricing) : 0;
    ...
    return React.createElement('div', { className: 'bsb-panel' },
      ...
      open ? React.createElement('div', { className: 'bsb-body' },
        ...
        React.createElement('div', { className: 'bsb-section' },
          React.createElement('div', { className: 'bsb-section-title' }, '花费 / 余额' + (pricing && pricing.modelId ? ' · ' + pricing.modelId : '')),
          React.createElement('div', { className: 'bsb-tiles' },
